...
Claude Science's corrected provider review v2 is preserved, with its material corrections and residual disagreements recorded in the [provider planning decision](PROVIDER_DECISION.md). The next bounded check concerns Sol's Chat Completions contract: its schema offers an optional, deprecated fingerprint field, but actual Sol behavior and endpoint-specific token accounting are unmeasured. No provider is selected and no amendment is adopted. The prior Responses/Messages inspection does not establish impossibility across every endpoint.
