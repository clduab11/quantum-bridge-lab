...
PROBES = ("gate_probe.py", "deadline_probe.py")
...
def main(argv=None):
    ...
    with tempfile.TemporaryDirectory(prefix="qbridge-e9-rev2-probes-") as temporary:
    ...
    print(
        json.dumps(
            {key: value for key, value in summary.items() if not key.endswith("sha256")},
            indent=2,
            sort_keys=True,
        )
    )
    return (
        0
        if summary["preserved_submission_unchanged"]
