"""Build amendment A1 rev2: verbatim old blocks are extracted from the v0.4 bytes and each
replacement is asserted to match exactly once. Produces the amendment document, a fully
applied CANDIDATE v0.5 text (not adopted), and a unified diff."""
...
SRC = "sources/specification__ai_quantum_control_protocol_v0.4.md"
V04 = Path(SRC).read_text(encoding="utf-8")
assert hashlib.sha256(V04.encode("utf-8")).hexdigest() == (
    "0df9242fd2d0ad5d30b75ea91f498842ac68fe0df9616afd4c9c04518233eda5"
)
lines = V04.split("\n")


def block(start_prefix, end_prefix=None, *, inclusive_end=True):
    """Return the exact text from the unique line starting with start_prefix up to (and
    including, if inclusive_end) the next line starting with end_prefix."""
    idx = [i for i, line_text in enumerate(lines) if line_text.startswith(start_prefix)]
    assert len(idx) == 1, (start_prefix, idx)
    i = idx[0]
    if end_prefix is None:
        return lines[i]
    j = next(k for k in range(i + 1, len(lines)) if lines[k].startswith(end_prefix))
    return "\n".join(lines[i : j + 1] if inclusive_end else lines[i:j])


L = "272,000"  # Codex-selected prospective admission limit (see review §3)

# ------------------------------------------------------------------ replacements
R = []  # (section label, old, new)

# Header line and status block
old = lines[0]
new = "# AI-Guided vs Conventional Parameter Discovery on a Simulated Single-Qubit Robust-Gate Objective — Protocol Draft v0.5 (CANDIDATE — amendment A1 applied for review; NOT ADOPTED)"
R.append(("Title", old, new))

old = block("Revision: protocol-v0.4 draft")
...
R.append(("Revision line", old, new))

# §6.1
old = block("### 6.1 Model and decoding", "- No tools, no system-side code execution")
new = """### 6.1 Model and decoding
...
R.append(("§7.1 additions", old, new))

# §7.2 whole section (six bullets) + new §7.2a
old = block(
    "### 7.2 Per-block caps (LLM arm)",
    "- HTTP timeout/backoff allowance alone is at most",
)
assert old.count("\n- ") == 6, old.count("\n- ")
deadline_bullet = block("- HTTP timeout/backoff allowance alone is at most")
deadline_tail = deadline_bullet.split("it is not a bound on total runtime. ", 1)[
    1
].replace(
    "Enforce an independent monotonic **36,000 s (10 h) deadline per arm per block**,",
    "The independent monotonic **36,000 s (10 h) deadline per arm per block** is the binding bound and is unchanged; enforce it,",
    1,
)
new = f"""### 7.2 Per-block caps (LLM arm)
...
- Output tokens ≤ 8192 per generation attempt (§6.1). **Input admission is enforced, not inferred:** every distinct logical-call body is counted by the provider before generation and a generation request is dispatched only if `input_tokens ≤ L*`, with **L\\* = {L} provider-counted input tokens** (prospective admission choice anchored to the documented price-tier boundary "prompts with >272K input tokens", read conservatively as 272,000; it is not a proof that every legal history fits and not spending authority; the earlier candidate value 200,000 is recorded as superseded in the amendment file). The visible history is at most 10·b valid rows at batch b ≤ 19 under §6.3 (forfeits and invalid evaluations reduce it; never more than 190 rows because batch 19's results are not fed back), but no row count, byte count or tokenizer estimate is used for any bound; whether and when a body exceeds L\\* depends on the rendered text, not only on row count, and no monotonicity in batch index is claimed. An over-limit body is a forfeited batch under §7.4. The per-attempt token ceiling is L\\* + 8,192; the a priori study ceiling is 4,560 × (L\\* + 8,192) generation tokens plus ≤ 4,560 count requests whose billing is unknown until account evidence exists. Provider-reported token categories are recorded separately; unavailable billing quantities remain unknown, never zero.
- Nominal fixed-backoff HTTP allowance (no server-imposed waits): 2 × (114×300 + 38×25) = 70,300 s per block, which already exceeds the arm deadline; with `Retry-After` waits it is not an upper bound at all, and it is not a bound on total runtime. {deadline_tail}
...
R.append(("§7.2 + new §7.2a", old, new))

# §7.3 whole section
old = block("### 7.3 Ordering of transport retry", "For each batch: **logical call 1**")
new = """### 7.3 Ordering of transport retry and malformed-output correction
...
R.append(("§7.3", old, new))

# §7.4 rows: replace the fingerprint row and add rows
old = block("| Provider model/fingerprint or effective decoding change;")
...
R.append(("§7.4 rows", old, new))

# §7.5 append
old = block("Durably reserve the allotted slot or transport-attempt number")
new = (
    old
    + " Count-class attempt numbers are durably reserved before dispatch exactly as generation attempts are, and the bound canonical request bytes are journaled before dispatch so that a process kill after dispatch leaves an auditable record; an unresolved count or generation reservation is logged as dispatch/execution status unknown with unknown billing, never zero. Retained headers exclude request authorization material."
)
R.append(("§7.5 addition", old, new))

# §8.3 stationarity bullet
old = block("- **Stationarity assumption:**")
...
R.append(("§8.3", old, new))

# §9.6
old = block("Per stage: all 20 D values;")
...
R.append(("§9.6", old, new))

# §11 E9
old = block("- **E9 LLM interface:**")
...
R.append(("§12 G-MODEL", old, new))
old = block("| G-TRANSPORT |")
...
R.append(("§12 G-TRANSPORT", old, new))
old = block("| G-COST |")
...
R.append(("§12 G-COST", old, new))

# §14.4 null/inconclusive
old = block("*For a null or inconclusive result:*")
new = old.replace(
    "forfeits under the failure contract dominate the endpoint;",
    "forfeits under the failure contract dominate the endpoint, including count-class failures, admission rejections and halts that exist only for the LLM arm;",
)
assert new != old
R.append(("§14.4", old, new))

# ------------------------------------------------------------------ apply
text = V04
for label, o, n in R:
    assert text.count(o) == 1, (label, text.count(o))
    text = text.replace(o, n)
Path("ai_quantum_control_protocol_v0.5_CANDIDATE_A1_2026-09-08.md").write_text(
    text, encoding="utf-8"
)
diff = "".join(
    difflib.unified_diff(
        V04.splitlines(keepends=True),
        text.splitlines(keepends=True),
        fromfile="specification/ai_quantum_control_protocol_v0.4.md (0df9242f…)",
        tofile="ai_quantum_control_protocol_v0.5_CANDIDATE_A1_2026-09-08.md",
        n=1,
    )
)
Path("v0.4_to_v0.5_CANDIDATE_A1.diff").write_text(diff, encoding="utf-8")

# ------------------------------------------------------------- amendment doc
cand_sha = hashlib.sha256(text.encode("utf-8")).hexdigest()
diff_sha = hashlib.sha256(diff.encode("utf-8")).hexdigest()
head = Path("amendment_head_v2.md").read_text(encoding="utf-8")
head = (
    head.replace("{CAND_SHA}", cand_sha)
    .replace("{DIFF_SHA}", diff_sha)
    .replace("{CAND_BYTES}", str(len(text.encode("utf-8"))))
)
parts = [head]
for i, (label, o, n) in enumerate(R, 1):
    parts.append(
        f"\n## A1.{i} — {label}\n\n**old (verbatim from v0.4)**\n\n```\n{o}\n```\n\n**new (complete replacement text)**\n\n```\n{n}\n```\n"
    )
tail = Path("amendment_tail_v2.md").read_text(encoding="utf-8")
parts.append(tail)
doc = "".join(parts)
Path("amendment_A1_counted_responses_admission_PROPOSED_2026-09-08.md").write_text(
    doc, encoding="utf-8"
)
print("candidate", cand_sha, len(text.encode()))
print("diff", diff_sha, len(diff))
print("amendment", hashlib.sha256(doc.encode()).hexdigest(), len(doc.encode()))
print("replacements", len(R), [r[0] for r in R])
print("verify", all(re.search(re.escape(n), text) for _, _, n in R))
