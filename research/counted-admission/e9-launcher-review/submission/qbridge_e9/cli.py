"""Executable E9 launcher. The offline dry run is the default.
    ...
    python -m qbridge_e9.cli gate --authority <file> --records <dir> ...

    # 3. FULL offline dry run (default mode): fabricated responses, no key,
    #    no network, complete orchestration and report
    python -m qbridge_e9.cli dry-run --fixtures <dir> --work-dir <dir>

    # 4. future gated live run: refuses unless every gate requirement passes
    python -m qbridge_e9.cli live --fixtures <dir> --work-dir <dir> \
        --authority <file> --adoption-evidence <file> \
...
from qbridge_e9.gate import API_KEY_ENV, GateFailed, evaluate_gate
...
def _common(parser):
    ...
    parser.add_argument("--repo-root", default=None, help="verify builder/source hashes")
    parser.add_argument("--work-dir", required=True, help="private directory OUTSIDE Git")
    parser.add_argument("--effort", default="medium")
    parser.add_argument("--usd-ceiling", default="25", help="E9-only USD ceiling (proposal)")
    parser.add_argument("--count-fee-ceiling", default="1.088",
                        help="USD ceiling for ONE count attempt incl. failed/rejected")
...
def _transport_factory(mode, fixture_set):
    if mode == "live":
        import httpx

        return (lambda: httpx.HTTPTransport(retries=0)), None
    scripted, counts = mocks.nominal_plan(fixture_set)
    import httpx

    return (lambda: httpx.MockTransport(scripted)), (scripted, counts)
...
def run_mode(args, mode):
    ...
    with DurableJournal(journal_path) as journal:
        records = RequestRecords(records_dir, public_repo=work / "public_repo_guard")
        fixture_set = load_fixture_set(
            args.fixtures, reasoning_effort=limits.reasoning_effort, repo_root=args.repo_root
        )
        transport_factory, _scripted = _transport_factory(mode, fixture_set)
...
def main(argv=None):
    ...
    dry = sub.add_parser("dry-run", help="offline dry run with FABRICATED responses (default)")
    _common(dry)
    ...
    report, path = run_mode(args, "dry-run")
    print(
        json.dumps(
            {
                "mode": report["mode"],
