...
* this orchestrator owns the fixture sequence, the two attempt caps, the
  attempt wall clock, the retry rule, the monotonic stop limit, admission, the
  count receipt and the durable no-replay behaviour.

Review findings fixed in this revision:

* **1** - the attempt wall clock is now enforced at the transport boundary by
  :mod:`qbridge_e9.deadline`: a budget is armed before every dispatch, the
  inner transport is cancelled at expiry, and a late result is rejected and
  recorded rather than accepted.
* **2** - the 7,400 s stop limit no longer restarts on reopen. Consumed time is
  persisted per run and subtracted, and any run whose journal already holds a
  terminal stop or a provider halt is REFUSED rather than continued.
* **5** - generation dispatch additionally requires conservative headroom for
  every applicable billing category, independently of whether the provider
  patch that fixes its own reservation has been applied.
* **7** - a fixture already finished in this journal has its outcome
  reconstructed from the durable events, so a rerun that sends nothing reports
  the same acceptance rather than silently losing it.

The orchestrator never lowers a server-directed wait, never retries a
documented billing/spend rejection, and never replays an attempt whose dispatch
outcome is unknown.
"""
...
from qbridge_e9.deadline import DeadlineGuard
...
# therefore classifies them itself. See
...
# Journal event kinds that make a journal terminal: no further dispatch.
TERMINAL_KINDS = ("e9_stopped", "provider_halted")
...
class LogicalOutcome:
    ...
    def as_dict(self):
        return {
            ...
            "order_batch": self.batch,
            "logical": self.logical,
            "outcome": self.outcome,
            "reason": self.reason,
            "reconstructed_from_journal": self.reconstructed,
...
class E9Orchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self._prior_consumed = self.consumed_seconds()
        self._run_index = self.run_index()
    ...
    def terminal_records(self):
        """Every persisted terminal stop or provider halt in this journal."""
        return [
            {"kind": e["kind"], "reason": e.get("reason")}
    ...
    def run_index(self):
        """1-based index of THIS run within the journal."""
        return 1 + sum(1 for e in self._events() if e["kind"] == "e9_run_started")

    def consumed_seconds(self):
        """Deadline time already consumed by earlier runs in this journal.

        Consumed time is recorded incrementally on every ``e9_logical_finished``
        as well as on ``e9_stopped`` / ``e9_run_finished``, so a run killed
        between fixtures still contributes the time it had already used instead
        of silently handing a resumed run the whole budget again. Within one run
        the elapsed clock is monotone, so the largest recorded value is that
        run's consumption; the totals are then summed across runs.
        """
        per_run = {}
        for event in self._events():
            if event["kind"] not in ("e9_run_finished", "e9_stopped", "e9_logical_finished"):
                continue
            index = event.get("run_index")
            if index is None:
                continue
            value = float(event.get("consumed_seconds") or 0.0)
            per_run[index] = max(per_run.get(index, 0.0), value)
        return sum(per_run.values())

    def budget_seconds(self):
        """The remaining share of the governing deadline for THIS run."""
        return self.limits.deadline_seconds - self._prior_consumed

    def elapsed_seconds(self):
        if self.started_monotonic is None:
            return 0.0
        return self.clock() - self.started_monotonic
    ...
    # -- reconstruction (review finding 7) --------------------------------
    def _reconstruct(self, fixture):
        """Rebuild a finished fixture's outcome from the durable journal."""
        events = self._events()
        finished = self._finished_events().get(fixture.name, {})
        outcome = LogicalOutcome(fixture.name, fixture.batch, fixture.logical)
        outcome.outcome = finished.get("outcome", "unknown")
        outcome.reason = finished.get("reason")
        ...
        for event in events:
            if event["kind"] == "count_receipt" and event.get("fixture") == fixture.name:
                outcome.counted_tokens = event.get("counted_tokens")
            elif event["kind"] == "e9_parse_result" and event.get("fixture") == fixture.name:
                outcome.parse = {
                    k: event.get(k)
                    for k in ("valid_count", "schema_valid", "reason", "vector_slots")
                }
            elif event["kind"] == "request_not_sent" and event.get("fixture") == fixture.name:
                outcome.admission = event.get("reason")
            elif event["kind"] == "completed":
                origin = reserved.get(event.get("reservation"), {})
                if origin.get("fixture") != fixture.name:
                    continue
                request_class = event.get("request_class")
                meta = event.get("metadata") or {}
                if request_class == "count":
                    outcome.count_attempts += 1
                    outcome.count_category = event.get("status")
                else:
                    outcome.generation_attempts += 1
                    outcome.generation_category = event.get("status")
                    outcome.usage = event.get("usage")
                    outcome.text = event.get("text")
                    outcome.identity_vector = meta.get("identity_vector")
                    outcome.generation_elapsed_seconds = meta.get("elapsed_seconds")
                    if origin.get("dispatch_monotonic") is not None:
                        outcome.generation_started_monotonic = origin["dispatch_monotonic"]
                    outcome.findings = (
                        list(meta.get("usage_findings") or [])
                        + list(meta.get("profile_findings") or [])
                        + list(meta.get("identity_findings") or [])
                    )
        if outcome.counted_tokens is not None and outcome.admission is None:
            outcome.admission = C.admit(outcome.counted_tokens, self.limits.admission_limit)
        return outcome
    ...
    def run(self):
        ...
        terminal = self.terminal_records()
        finished_events = self._finished_events()
        all_finished = all(f.name in finished_events for f in self.fixture_set.fixtures)
        ...
        self.journal.append(
            ...
            prior_consumed_seconds=self._prior_consumed,
            ...
            prior_terminal_records=terminal,
            deadline_guard_armed=self.guard is not None,
        )

        # --- refusal conditions, evaluated before any dispatch -----------
        refusal = None
        if terminal and not all_finished:
            # Prefer the launcher's own recorded stop over a provider halt, so
            # the refusal reason does not depend on whether patch 0001 makes
            # the provider classify a billing 429 as a credential halt first.
            own = next((t for t in terminal if t["kind"] == "e9_stopped"), None)
            refusal = f"prior_terminal_stop:{(own or terminal[0])['reason']}"
        elif unresolved:
            # An attempt whose dispatch outcome is unknown may already have
            # been billed. Replaying it could duplicate a dispatch.
            refusal = f"unresolved_prior_attempt:{unresolved[0]}"
        elif self.budget_seconds() <= 0:
            refusal = "governing_deadline_already_consumed"
        elif self.guard is None:
            refusal = "attempt_wall_clock_not_enforceable"

        if refusal is not None and not all_finished:
            self.refused_reason = refusal
            self._stop_reason = refusal
            self.journal.append(
                "e9_run_refused", arm=E9_ARM, block=E9_BLOCK, reason=refusal, dispatched=False
            )
            for fixture in self.fixture_set.fixtures:
                outcome = LogicalOutcome(fixture.name, fixture.batch, fixture.logical)
                if fixture.name in finished_events:
                    outcome = self._reconstruct(fixture)
                else:
                    outcome.outcome, outcome.reason = "refused", refusal
        ...
        for fixture in self.fixture_set.fixtures:
            if fixture.name in finished_events:
                # Review finding 7: reconstruct rather than discard.
                self.outcomes.append(self._reconstruct(fixture))
                continue
            outcome = LogicalOutcome(fixture.name, fixture.batch, fixture.logical)
            self.outcomes.append(outcome)
            ...
            self.journal.append(
                ...
                block=E9_BLOCK,
                run_index=self._run_index,
                consumed_seconds=self.elapsed_seconds(),
    ...
    def _logical(self, fixture, outcome):
        ...
        if admission != "admitted":
            ...
            self._not_sent(fixture, "admission_rejected", counted_tokens=result.counted_tokens)
            return
        ...
        # Review finding 5: require conservative headroom before dispatch.
        rates = self.provider.authority.rates()
        conservative = conservative_generation_reservation(result.counted_tokens, rates)
        state = self.provider.ledger.state()
        headroom = state["ceiling"] - state["committed"]
        if conservative > headroom:
            outcome.outcome = "generation_blocked"
            outcome.reason = "insufficient_conservative_headroom"
            self._not_sent(
                fixture,
                "insufficient_conservative_headroom",
                conservative_required_usd=C.money_str(conservative),
                headroom_usd=C.money_str(headroom),
                counted_tokens=result.counted_tokens,
            )
            self._persist_stop("insufficient_conservative_headroom")
            return
        ...
        outcome.outcome = (
            "completed"
            if gen_result.category == "received_completed"
            else ("incomplete" if gen_result.category == "received_incomplete"
    ...
    def _attempts(self, pair, receipt, fixture, request_class, outcome):
        ...
        for attempt in range(1, self.limits.max_attempts_per_logical + 1):
            ...
            reservation = self.journal.reserve(
                ...
                fixture=fixture.name,
                dispatch_monotonic=started,
                armed_budget_seconds=timeout,
            )
            ...
            # Review finding 1: arm the wall clock before the only wire path.
            ...
            try:
                ...
                self.guard.disarm()
                boundary = self.guard.drain()
            if boundary["expiries"] or boundary["late_results"]:
                self.journal.append(
                    "e9_attempt_wall_clock",
                    arm=E9_ARM,
                    block=E9_BLOCK,
                    batch=fixture.batch,
                    logical=fixture.logical,
                    ...
                    attempt=attempt,
                    armed_budget_seconds=timeout,
                    expiries=boundary["expiries"],
                    late_results_rejected=boundary["late_results"],
                    cancellations=boundary["cancellations"],
                    workers_still_alive=self.guard.workers_alive,
                )
